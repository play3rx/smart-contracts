...
async function main() {
  ...
  // Make a request...
  const size = 5;
  const receipt = await QrngRequesterContract.makeRequestUint256Array(size, {gasLimit: 500000});
  ...
  const log = await new Promise((resolve) =>
    hre.ethers.provider.once(QrngRequesterContract.filters.ReceivedUint256Array(requestId, null), resolve)
  );
  const parsedLog = QrngRequesterContract.interface.parseLog(log);
